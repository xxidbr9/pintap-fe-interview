...
const CartSection = (props: Props) => {
  ...
  const _handleAddQuantity = (id: string) => {
    const finnedProduct = rdxProductInCart.find(product => product.id === id)
    const quantity = finnedProduct.quantity + 1
    if (quantity <= finnedProduct.stock) dispatch(rdxCartActions.setQuantity({ id, quantity }))
  }
          ...
          <div className="flex gap-x-4" key={cartProduct.id}>
            <div className="w-24 h-24 border rounded-md">
              {!!cartProduct.thumbnail && (
                <Image src={cartProduct.thumbnail} layout="fixed" width={96} height={96} className="object-cover w-full" alt={cartProduct.title} />
              )}
              {!cartProduct.thumbnail && (
            ...
            <div className="flex-1 flex flex-col justify-between py-1">
              <div className="flex flex-col gap-y-1">
                <span className="flex-1 text-ellipsis overflow-hidden font-medium" title={cartProduct.title}>
                  {cartProduct.title}
                </span>
              ...
              <div className="flex justify-between">
                <div className="flex gap-x-2 items-center border px-2 rounded-sm">
                  <ButtonCart onClick={() => _handleSubQuantity(cartProduct.id as string)}>-</ButtonCart>
                  <div>{cartProduct.quantity}</div>
                  <ButtonCart
                    disabled={cartProduct.stock <= cartProduct.quantity}
                    onClick={() => _handleAddQuantity(cartProduct.id as string)}>
                    +
                  </ButtonCart>
                </div>
                <button
                  onClick={() => _handleRemoveClick(cartProduct.id as string)}>
                  <TrashIcon color={colors.red[500]} />
                </button>
              </div>
            </div>
...
const ButtonCart = styled.button<ButtonCartProps>`
  font-size: 24px;
  ${props => props.disabled ? tw`text-gray-200` : tw`text-gray-600`}
`
