...
import { Product, ProductListEntity } from '@domain/product/entities/iProductList.entity';
...
const ProductDetailSection = (props: Props) => {
  ...
  const _handleCartProductDetailClick = (product: Product) => {
    if (props.isModal) props.onCartClick()
    if (!props.isModal) dispatch(rdxCartActions.setProductList(product))
  }
    ...
    <Container className={`${props.isModal ? "" : "laptop:mb-20 mobile:mb-10"}`}>
      <div className='flex gap-x-8 laptop:flex-row mobile:flex-col'>
        ...
        <div className='flex flex-col gap-y-4 mobile:py-4'>
          <div>
            <h1 className='text-lg font-medium'>{props.data.title}</h1>
