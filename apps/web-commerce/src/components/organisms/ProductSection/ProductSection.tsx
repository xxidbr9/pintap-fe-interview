...
import { ProductListEntity, Product } from '@domain/product/entities/iProductList.entity'
...
type ProductType = Product & { quantity: number }
...
const ProductSection = (props: Props) => {
  ...
  const otherProducts = products.map(product => ({ ...product, slug: createSlugLink(RoutesType.HOME, product.title, product.id as string) }))
          ...
          <div className='grid desktop:grid-cols-4 laptop:grid-cols-2 tablet:grid-cols-3 tablet:gap-x-6 mobile:grid-cols-2 laptop:gap-x-6 laptop:gap-y-8 mobile:gap-y-4 mobile:gap-x-4'>
            {products.map((product) => (
              <ProductCard
                ...
                data={{ ...product, slug: createSlugLink(RoutesType.HOME, product.title, product.id as string), quantity: product.quantity }}
