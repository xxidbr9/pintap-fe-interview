...
const ProductCard: React.FC<ProductCardProps> = ({ data: product, otherProducts, ...props }) => {
  ...
  const _handleLinkEvent = (event) => {
    ...
    dispatch(rdxProductThunkActions.fetchProductDetail(product.id as string))
    setIsOpen(true)
  }
  ...
  const _handlePrevProduct = () => {
    if (thisProductIndex > 0) {
      ...
      dispatch(rdxProductThunkActions.fetchProductDetail(nextProduct.id as string))
  ...
  const _handleAddToCart = () => props.onCartClick(product)
      ...
      <div className="border rounded flex flex-col">
        <a href={product.slug} onClick={_handleLinkEvent}>
          <Image
            ...
            src={!!src ? src : NotFoundImage.src} layout="responsive" width={IMAGE_WIDTH} height={IMAGE_HEIGHT} className="object-cover w-full" alt={product.title} />
        </a>
        <div className='px-2 border-t py-2 flex flex-col justify-between flex-1 gap-2 bg-gray-50 relative'>
          <a href={product.slug} onClick={_handleLinkEvent} >
            <div className="absolute w-full h-full z-10" />
            <span className='font-medium' dangerouslySetInnerHTML={{ __html: product.title }} />
          </a>
